...
class Forward_list {
   ...
   public:
    ...
    Forward_list(const Forward_list& o) : Forward_list() { add_all(o); }
...
struct Forward_list<T>::Node {
    ...
    template <typename... Args>
    Node(Args&&... args)
        : value_(std::forward<Args>(args)...), next_(nullptr) {}
};
...
void Forward_list<T>::merge_sort() {
    ...
    List l(head_, 0);
    for (Node* node = l.head_; node; node = node->next_) ++l.length_;
    ...
    static struct {
        List merge(const List& l1, const List& l2) {
            ...
            Node* n = l.head_;
            while (n1 && n2) n = (n->next_ = append_node());
            if (n1)
                n->next_ = n1;
            else
                n->next_ = n2;
