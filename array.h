...
class Array {
   ...
   public:
    ...
    Array(const std::initializer_list<T>& i_list) : Array(i_list.size()) {
        size_t i = -1;
        for (auto& el : i_list) ptr_[++i] = el;
    }

    Array(const Array& o) : Array(o.size_) {
        for (size_t i = 0; i < size_; ++i) ptr_[i] = o.ptr_[i];
    }
    ...
    static auto build_array(Args&&... args) {
        return Array_builder<Array<T>, T>::build_array(
...
class Array<bool> {
    ...
    class Reference {
       ...
       public:
        Reference(bool value)
            : value_(value), char_ptr_(nullptr), bit_index_(0) {}
   ...
   private:
    ...
    class Base_iterator {
       private:
        ...
        Base_iterator(unsigned char* ptr, size_t index)
            : ptr_(ptr), index_(index) {}

       public:
        void operator++() { ++index_; };
        bool operator==(const Base_iterator& o) { return index_ == o.index_; }
        bool operator!=(const Base_iterator& o) { return index_ != o.index_; }
        R& operator*() {
   ...
   public:
    ...
    Array(const Array& o)
        : actual_size_(o.actual_size_),
          ptr_(new unsigned char[actual_size_]),
          size_(o.size_) {
        auto p = ptr_;
        auto p_o = o.ptr_;
    ...
    Array(Array&& o)
        : actual_size_(o.actual_size_), ptr_(o.ptr_), size_(o.size_) {
        o.ptr_ = nullptr;
    }
    Array& operator=(Array&& o) {
    ...
    const_iterator cbegin() const { return const_iterator(ptr_, 0); };
    const_iterator cend() const { return const_iterator(ptr_, size_); };
    template <typename... Args>
    static Array<bool> build_array(Args&&... args) {
        return Array_builder<Array<bool>, bool>::build_array(
            std::forward<Args>(args)...);
    }
};
