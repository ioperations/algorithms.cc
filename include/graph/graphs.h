...
auto construct_euler_tour_sample(C& constructor) {
    for (int i = 0; i < 7; ++i) constructor.get_or_create_vertex(i);
    constructor.add_edge(0, 1)
        .add_edge(0, 2)
        .add_edge(0, 5)
        .add_edge(0, 6)
        .add_edge(1, 2)
...
auto hamilton_path_sample() {
    ...
    auto constructor = Graph::Constructor(graph);
    construct_euler_tour_sample(constructor);
    constructor.add_edge(1, 3).add_edge(3, 5);
    return graph;
}
...
G strong_components_sample() {
    Graph::Builder<G> b;
    for (int i = 0; i < 13; ++i) b.for_vertex(i);
...
G weighted_graph_sample() {
    Graph::Builder<G> b;
    for (int i = 0; i < 8; ++i) b.for_vertex(i);
    return b.for_vertex(0)
        .add_edge(1, .32)
        .add_edge(2, .29)
        .add_edge(5, .60)
...
G weighted_dag_sample() {
    Graph::Builder<G> b;
    for (int i = 0; i < 9; ++i) b.for_vertex(i);
    return b.for_vertex(0)
        .add_edge(1, .41)
        .add_edge(7, .41)
        .add_edge(9, .41)
