...
class Vertex_positions_impl {
   private:
    ...
    using Positions_map = boost::iterator_property_map<
        Positions::iterator, boost::property_map<Calculator::graph_type,
                                                 boost::vertex_index_t>::type>;
    Calculator& calculator_;
    Positions positions_;

    Vertex_position compose_position(const Calculator::vertex_descriptor& v) {
        return {b::get(b::vertex_name, calculator_.graph_, v), map_[v][0],
                map_[v][1]};
    }
   ...
   public:
    ...
    void update() {
        ...
        map_ = Positions_map(positions_.begin(),
                             get(b::vertex_index, calculator_.graph_));
    }

    template <typename F>
    void iterate(F f) {
        b::graph_traits<Calculator::graph_type>::vertex_iterator vi, vi_end;
        for (b::tie(vi, vi_end) = b::vertices(calculator_.graph_); vi != vi_end;
             ++vi) {
    ...
    void iterate_edges(F f) {
        ...
        for (b::tie(vi, vi_end) = b::edges(calculator_.graph_); vi != vi_end;
             ++vi) {
...
Vertex_positions::Vertex_positions(Calculator& g)
    : positions_impl_(new Vertex_positions_impl(g)) {}
...
void Vertex_positions::iterate_edges(
    std::function<void(const std::pair<Vertex_position, Vertex_position>&)> f) {
...
Calculator& Calculator::add_edge(const vertex_descriptor& v1,
...
Layout Calculator::calculate_layout_2(double size,
                                      int iterations) {  // todo rename
    ...
    Positions_map positions_map(positions.begin(),
                                get(b::vertex_index, graph_));

    b::minstd_rand generator;
    topology_type topology(generator, size);
    b::random_graph_layout(graph_, positions_map, topology);
    b::fruchterman_reingold_force_directed_layout(graph_, positions_map,
                                                  topology);
    ...
    class Composer {
       ...
       public:
        ...
        auto find(const vertex_descriptor& v) { return positions_map_.find(v); }
        auto map_end() { return positions_map_.end(); }
    ...
    for (b::tie(vi, vi_end) = b::edges(graph_); vi != vi_end; ++vi) {
        auto get_or_create_pos = [this, &composer, &positions_map](auto v) {
            ...
            if (entry == composer.map_end())
                position =
                    composer.add(v, {b::get(b::vertex_name, graph_, v),
                                     positions_map[v][0], positions_map[v][1]});
            else
